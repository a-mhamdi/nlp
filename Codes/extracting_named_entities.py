...
def _(nltk_ner):
    ...
    """
    _entities = nltk_ner(_text)
    for entity_type, entity_list in _entities.items():
        if entity_list:
...
def _(spacy):
    ...
    # uv run python -m spacy download en_core_web_sm  # <- CAUTION: It won't survive `uv sync --upgrade`
