...
@app.cell
def _():
    print(5 | 2, 5 or 2)   # bitwise vs. logical or
    print(5 & 2, 5 and 2)  # bitwise vs. logical and
    return
...
def _(mo):
    mo.md(r"""
    `try/except` lets us attempt risky code and handle the failure gracefully instead of crashing.
    ```python
    try:
        # code that might fail
    except SomeError:
        # runs if SomeError occurs
    except (AnotherError, YetAnother):
        # catch multiple exceptions
    except Exception as e:
        # catch anything else, e holds the error
    else:
        # runs ONLY if no exception occurred
    finally:
        # ALWAYS runs, error or not (cleanup goes here)
    ```
    """)
    return
...
def _(mo):
    mo.md("""
    ### Mutability
    Variables are labels pointing to objects. Mutable objects (list, dict, set) can be changed in-place, so multiple labels pointing to the same object will all "see" the change. Immutable objects (int, str, tuple) can never be changed in-place, so reassignment always creates a new object.
    """)
    return
...
def _():
    _a = [1, 2, 3]
    _b = _a          # _b points to the SAME list
    _b.append(4)
    print(_a)       # [1, 2, 3, 4] — _a is affected!
    return
...
def _():
    _a = [1, 2, 3]
    _b = _a.copy()   # _b is a NEW list
    _b.append(4)
    print(_a)       # [1, 2, 3] — _a is unaffected
    return
...
def _():
    ...
    _d2 = _d1
    _d2["y"] = 2
    print(_d1)      # {"x": 1, "y": 2} — _d1 is affected!
    return
...
def _(mo):
    mo.md("""
    4. Reassignment does NOT mutate
    """)
    return
...
def _():
    _a = [1, 2, 3]
    _b = _a
    _b = [9, 9, 9]  # _b now points to a NEW list
    print(_a)       # [1, 2, 3] — _a is unaffected
    return
...
def _():
    _a = [[1, 2], [3, 4]]
    _b = _a.copy()       # shallow copy
    _b[0].append(99)
    print(_a)           # [[1, 2, 99], [3, 4]] — inner list still shared!

    import copy
    _b = copy.deepcopy(_a)  # deep copy fixes this
    return
...
def _(mo):
    mo.md("""
    6. Integers are immutable — no surprise mutations
    """)
    return
...
def _():
    ...
    _y = _x
    _y += 1
    print(_x)       # 5 — ints are immutable, always safe
    return
...
def _():
    ...
    f(), f(), f()  # <- `x=[]` is created once, not on each call

    def f(x=None):  # set default to None
        x = x or []
        x.append(1)
        print(x)
...
def _(mo):
    mo.md("""
    **Example #3: Extracting Data from a Log File**
    """)
    return
...
def _(mo):
    ...
    A context manager is a way to automatically set up and tear down resources — like files, database connections, or locks — ensuring cleanup always happens even if an error occurs. It's the machinery behind the with statement.
    """)
    return
...
def _():
    ...
    with open("file.txt", "w") as _f:
        data = _f.write("This is the first sentence.\n")

    # manual equivalent (what Python does under the hood)
    _f = open("file.txt", "a")
    try:
        data = _f.write("A second statement!")
    finally:
        _f.close()   # always runs, even on error
    return
...
def _(mo):
    mo.md(r"""
    **Writing our own context manager with a function**

    Using a generator from `contextlib` — the `yield` splits setup and teardown.
    """)
    return
...
def _(contextmanager):
    ...
    def open_read_only(filename):
      ...
      # Yield read_only_file so it can be assigned to `_f`
      yield read_only_file
      # Close read_only_file
      read_only_file.close()

    with open_read_only('file.txt') as _f:
      print(_f.read())
    return
...
def _(mo):
    mo.md(r"""
    **Writing our own context manager with a class**

    We need two magic methods: `__enter__` and `__exit__`.
    """)
    return
...
def _():
    class OpenReadOnly:
        """Opens a file in read-only mode and ensures it is closed after use.
        ...
        def __enter__(self):
            ...
            return self.file                          # assigned to `_f`

        def __exit__(self, exc_type, exc_val, exc_tb):
            """Close the file, whether or not an error occurred.
    ...
    with OpenReadOnly('file.txt') as _f:
        print(_f.read())
    return
...
def _(mo):
    ...
    A decorator is a function that wraps another function to extend or modify its behavior without changing its source code. It's just syntactic sugar for passing a function as an argument to another function and replacing it with the result.
    """)
    return
